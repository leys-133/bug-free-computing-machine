...
const STUDENT = {
    ...
    checkConnection() {
        ...
        if (user.connectedTeacher) {
            ...
            if (teacher) {
                statusDiv.innerHTML = `<i class="fas fa-link"></i><span>مرتبط</span>`;
                statusDiv.classList.add('connected');

                joinCard.classList.add('hidden');
                connectedCard.classList.remove('hidden');
                document.getElementById('connected-teacher-name').textContent = teacher.name;
...
function joinRoom() {
    ...
    if (!code || code.length < 4) {
        errorDiv.textContent = 'يرجى إدخال رمز صحيح';
        errorDiv.classList.remove('hidden');
        return;
    }

    // Find teacher with this room code
    const users = AUTH.getUsers();
    const teacher = users.find(u => u.accountType === 'teacher' && u.roomCode === code);

    if (!teacher) {
        errorDiv.textContent = 'رمز الحلقة غير صحيح';
        errorDiv.classList.remove('hidden');
        return;
    }
